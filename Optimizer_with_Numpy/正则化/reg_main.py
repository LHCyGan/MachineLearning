...
# 加载数据
train_X, train_Y, test_X, test_Y = reg_utils.load_dataset(is_plot=False)
...
def plot(parameters):
    ...
    reg_utils.plot_desicion_boundary(lambda x: reg_utils.predict_dec(parameters, x.T), train_X, train_Y)


if __name__ == '__main__':
    # parameters = model(train_X, train_Y, is_plot=True)
    # plot(parameters)
    # parameters = model(train_X, train_Y, learning_rate=0.3, is_plot=True, lambd=0.7)
    # plot(parameters)
    parameters = model(train_Y, train_Y, learning_rate=0.3, is_plot=True, keep_prob=0.5)
    plot(parameters)